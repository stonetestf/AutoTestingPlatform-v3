        ...
        <div>
            <el-form :inline="true" v-if="SelectRomeData.disPlay"  method="post">
                ...
                <el-form-item label="接口名称:">
                    <el-input clearable v-model.trim="SelectRomeData.apiName"></el-input>
                </el-form-item>
                        ...
                        <el-form v-if="props.row.tableItem.length!=0">
                            <el-table
                                ...
                                <el-table-column 
                                    ...
                                    align="center">
                                    <template slot-scope="scope">
                                        <div style="white-space: pre-line;text-align: left;" v-html="scope.row.textInfo"></div>
                                    </template>
                                </el-table-column>
                            </el-table>
                        </el-form>
...
export default {
    ...
    methods: {
        ...
        SelectData(){
            ...
            self.loading=true;
            self.tableData= [];
            self.$axios.get('/api/ApiIntMaintenance/SelectHistory',{
                params:{
                    'sysType':'API',
        ...
        RestorData(historyId){//恢复数据
            let self = this;
            self.$axios.post('/api/ApiIntMaintenance/RestorData',Qs.stringify({
                "historyId":historyId,
            })).then(res => {
                if(res.data.statusCode==2002){
                    self.$message.success('接口恢复成功!');
                    self.dialogVisible = false;//关闭新增弹窗
                    self.$emit('closeDialog');
                    self.$emit('Succeed');//回调查询
                }
                else{
                    self.$message.error('接口恢复失败:'+res.data.errorMsg);
                }
            }).catch(function (error) {
                console.log(error);
            })
